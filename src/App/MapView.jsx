...
export default function MapView({
  ...
  etaMinutes, distToDropoff, getStatusMsg,
}) {
  return (
    <div style={{
        ...
        <>
          <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
            <line
              ...
              x2={`${['picked_up','heading_to_dropoff','arrived_at_dropoff'].includes(rideStatus) ? dropoffCoords.x : pickupCoords.x}%`}
              y2={`${['picked_up','heading_to_dropoff','arrived_at_dropoff'].includes(rideStatus) ? dropoffCoords.y : pickupCoords.y}%`}
      ...
      {pickupCoords && dropoffCoords && !isTracking && fareData && tripData && (
        <div style={{ position: 'absolute', top: '12px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(255,255,255,.97)', backdropFilter: 'blur(12px)', border: '1px solid #E5E7EB', borderRadius: '14px', padding: '10px 16px', display: 'flex', alignItems: 'center', gap: '14px', whiteSpace: 'nowrap', boxShadow: '0 4px 16px rgba(0,0,0,.07)', animation: 'scaleIn .3s ease-out' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
            <span style={{ fontSize: '13px', fontWeight: 700, color: '#111827', fontFamily: '"JetBrains Mono",monospace' }}>{tripData.actualMiles} mi</span>
            <span style={{ fontSize: '11px', color: '#9CA3AF' }}>({tripData.actualKm} km)</span>
          </div>
          <div style={{ width: '1px', height: '16px', background: '#E5E7EB' }}/>
          <span style={{ fontSize: '12px', fontWeight: 700, color: '#111827' }}>{tripData.totalMin} min</span>
          <div style={{ width: '1px', height: '16px', background: '#E5E7EB' }}/>
          <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
            <div style={{ width: '7px', height: '7px', borderRadius: '50%', background: tripData.traffic.color }}/>
            <span style={{ fontSize: '12px', fontWeight: 700, color: tripData.traffic.color }}>{tripData.traffic.label}</span>
          </div>
        </div>
      )}
    </div>
