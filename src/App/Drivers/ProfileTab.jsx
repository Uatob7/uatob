...
import {
  ...
  Camera, Loader2, CheckCircle2, RefreshCw,
} from 'lucide-react';
...
function UploadSlot({ slotKey, urlKey, label, Icon, docs, uid, onUploaded }) {
  ...
  const handleFile = useCallback(async (file) => {
    ...
    try {
      ...
      // Optimistic local update — onSnapshot will confirm shortly after
  ...
  return (
    <>
      ...
      <input
        ...
        capture={slotKey === "profilePhoto" ? "user" : undefined}
        style={{ display: "none" }}
      ...
      <div
        style={{
          background:   s.bg,
          border:       `1.5px solid ${uploading ? "#2563EB40" : s.border}`,
          borderRadius: 16,
      ...
      >
        ...
        <div style={{
          ...
          {uploading
            ? <Loader2 size={17} color="#2563EB" style={{ animation: "spin 1s linear infinite" }} />
            : isUploaded
              ? <CheckCircle2 size={17} color={s.iconColor} />
              : <Upload size={17} color={s.iconColor} />
          }
        </div>

        {/* Label */}
        <div className="condensed" style={{ fontSize: 13.5, fontWeight: 800, color: C.text, lineHeight: 1.2, marginBottom: 3 }}>
          {label}
        </div>
        ...
        <div className="mono" style={{
          ...
          color: uploading ? "#2563EB" : s.color,
          letterSpacing: ".05em",
          ...
          >
            ...
            <button
              ...
              style={{
                ...
                ...(isUploaded ? {
                  ...
                  background: `linear-gradient(135deg, ${s.iconBg}, rgba(220,38,38,.2))`,
            ...
            >
              {isUploaded ? (
                ...
                <>
                  <Upload size={12} color={s.iconColor} />
                  <span className="mono" style={{ fontSize: 10, fontWeight: 800, color: s.iconColor, letterSpacing: ".03em" }}>
                    {slotKey === "profilePhoto" ? "TAKE PHOTO" : "UPLOAD NOW"}
                  </span>
                </>
              )}
            </button>
...
const DocumentsInsuranceSection = ({ driver, onBack }) => {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(
      ...
      (err) => {
        console.error("[Docs] onSnapshot error:", err);
        setSyncing(false);
      }
    );
    return () => unsub();
  ...
  // Optimistic update — snapshot will confirm shortly after
  ...
  return (
    <div style={{ padding: "18px 20px", display: "flex", flexDirection: "column", gap: 14, animation: "slideUp .38s ease-out forwards" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        {syncing && (
          <RefreshCw size={13} color={C.textDim} style={{ animation: "spin 1s linear infinite", flexShrink: 0 }} />
        )}
      </div>
      ...
      <div style={{
        ...
        border:     `1.5px solid ${allDone ? "#86EFAC" : "#93C5FD"}`,
        borderRadius: 18, padding: "18px 16px",
        display: "flex", gap: 14, alignItems: "flex-start",
        boxShadow: allDone ? "0 4px 20px rgba(22,163,74,.1)" : "0 4px 20px rgba(37,99,235,.08)",
      }}>
        <div style={{
          ...
          {allDone
            ? <CheckCircle2 size={21} color="#fff" />
            : <Shield size={20} color="#fff" />
          }
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", marginBottom: 6 }}>
            <div className="condensed" style={{ fontSize: 16, fontWeight: 900, color: allDone ? "#14532D" : "#1E3A8A" }}>
              {uploaded}/{slots.length} documents submitted
            </div>
            <div className="mono" style={{ fontSize: 11, fontWeight: 700, color: allDone ? "#16A34A" : "#2563EB" }}>
              {pct}%
            </div>
          </div>
          ...
          <div style={{ fontSize: 12, color: allDone ? "#16A34A" : "#3B82F6", marginTop: 6, fontWeight: 500 }}>
            {allDone
              ...
              : `${slots.length - uploaded} document${slots.length - uploaded !== 1 ? "s" : ""} still needed — tap a card below to upload`
            }
          </div>
        </div>
      </div>
      ...
      {!allDone && (
        <div style={{
          background: "#FEF2F2", border: "1.5px solid rgba(220,38,38,.2)",
          borderRadius: 13, padding: "12px 16px",
          display: "flex", gap: 10, alignItems: "center",
        }}>
          <AlertCircle size={16} color={C.red} style={{ flexShrink: 0 }} />
          <div>
            <div className="condensed" style={{ fontSize: 13, fontWeight: 800, color: C.red, marginBottom: 2 }}>
              Action required
            </div>
            <div style={{ fontSize: 11.5, color: "#991B1B", fontWeight: 500, lineHeight: 1.4 }}>
              Missing documents will delay your account approval. Upload them now to get on the road faster.
            </div>
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        <span style={{ fontSize: 12, color: "#92400E", fontWeight: 500, lineHeight: 1.5 }}>
          Documents are reviewed by the UaTob team within 24–48 hours. You'll receive a notification once approved.
        </span>
      </div>
    </div>
  );
