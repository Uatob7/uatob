      ...
      <div ref={wrapRef} style={{ position:'relative' }}>
        <div style={{
          ...
          {onLocationRequest && !value && (
            <button type="button" onClick={onLocationRequest} style={{ background:'none', border:'none', cursor:'pointer', padding:0, color:C.dim, display:'flex', alignItems:'center', transition:'color .12s' }}
          ...
          {value && (
            <button type="button" onClick={() => { onChange(''); clearSug(); setGhost(''); }}
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:7 }}>
      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        {showBack && (
          <button onClick={onBack} style={{ background:'none', border:'none', cursor:'pointer', color:C.dim, padding:0, display:'flex', alignItems:'center', flexShrink:0 }}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          {step > 0 && step < 11 && (
            <button onClick={onReset} style={{ background:'rgba(255,255,255,.06)', border:'1px solid rgba(255,255,255,.1)', borderRadius:7, width:22, height:22, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
      ...
      {showRoute && (
        <div style={{ background:C.faint, border:`1px solid ${C.borderDim}`, borderRadius:9, padding:'7px 11px', display:'flex', flexDirection:'column', gap:4, animation:'brFadeIn .3s ease both' }}>
          {[
            ...
            <div key={i} style={{ display:'flex', alignItems:'center', gap:7 }}>
              ...
              <span style={{ fontFamily:MONO, fontSize:8.5, color:'rgba(255,255,255,.45)', whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:230 }}>{r.val}</span>
            </div>
          ))}
          {rideLabel && (
...
function DriverBanner({ driverInfo }) {
  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 11px', background:C.greenDim, border:`1px solid ${C.borderDim}`, borderRadius:9 }}>
      <div style={{ width:6, height:6, borderRadius:'50%', flexShrink:0, background: stale ? 'rgba(255,255,255,.3)' : C.greenBright, animation: stale ? 'none' : 'brBlink 1.6s ease-in-out infinite' }}/>
      <span style={{ fontFamily:MONO, fontSize:8.5, color: stale ? C.dim : C.greenBright, flex:1 }}>
        {stale ? `${count} driver nearby · estimated location` : `${count} driver${count !== 1 ? 's' : ''} nearby`}
      </span>
      {miles != null && <span style={{ fontFamily:MONO, fontSize:8, color:C.dim }}>{miles} mi</span>}
    </div>
  );
}
  ...
  return (
    <div style={{ border:`1px solid rgba(129,140,248,.28)`, borderRadius:13, background:'#0A1A0C', overflow:'hidden' }}>
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 13px 8px', borderBottom:`1px solid ${C.borderFaint}` }}>
        <button type="button" onClick={onPrevMonth} style={{ background:'none', border:'none', cursor:'pointer', color:C.dim, padding:4, borderRadius:7, display:'flex' }}><Ico n="back" size={13} color={C.dim}/></button>
        <span style={{ fontFamily:COND, fontSize:12, fontWeight:800, letterSpacing:'.06em', color:'#fff' }}>{MONTHS_FULL[selMonth]} {selYear}</span>
        <button type="button" onClick={onNextMonth} style={{ background:'none', border:'none', cursor:'pointer', color:C.dim, padding:4, borderRadius:7, display:'flex' }}><Ico n="fwd" size={13} color={C.dim}/></button>
      </div>
...
function PromoRow({ originalTotal, onDiscountChange }) {
  ...
  if (promo.discount) {
    return (
      <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'9px 12px', border:`1px solid ${C.border}`, borderRadius:10, background:C.greenDim, animation:'brFadeIn .25s ease both' }}>
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        ...
        <button type="button" onClick={promo.handleRemove} style={{ background:'none', border:'none', cursor:'pointer', color:C.dim, padding:3 }}>
          <Ico n="x" size={12}/>
        </button>
      </div>
    );
  ...
  if (!promo.open) {
    return (
      <button type="button" onClick={() => promo.setOpen(true)} style={{ background:'none', border:`1px dashed ${C.borderFaint}`, borderRadius:10, padding:'8px 12px', cursor:'pointer', width:'100%', display:'flex', alignItems:'center', gap:7, color:C.fade, fontFamily:MONO, fontSize:10, fontWeight:500 }}>
  ...
  return (
    <div>
      <div style={{ display:'flex', gap:6, border:`1px solid ${promo.error ? C.red : C.borderDim}`, borderRadius:10, overflow:'hidden', background:C.faint }}>
        ...
        <input
          ...
          style={{ flex:1, background:'none', border:'none', outline:'none', fontFamily:MONO, fontSize:11, color:'#fff', letterSpacing:'.1em', padding:'10px 8px', caretColor:C.greenBright }}
        />
        <button type="button" onClick={promo.handleApply} disabled={!promo.code.trim() || promo.loading}
          style={{ padding:'0 14px', background: promo.code.trim() && !promo.loading ? C.green : C.faint, border:'none', cursor: promo.code.trim() && !promo.loading ? 'pointer' : 'not-allowed', fontFamily:COND, fontWeight:800, fontSize:10.5, letterSpacing:'.1em', color: promo.code.trim() && !promo.loading ? '#fff' : C.fade, flexShrink:0, display:'flex', alignItems:'center', justifyContent:'center' }}>
...
function BookRideCardInner({ uid, onRequest, onBookingComplete, onActiveChange }) {
  ...
  const bookingPayload = useMemo(() => {
    ...
    return {
      ...
      scheduledAt, promoDiscount, grandTotal, driverInfo]);
  ...
  const renderLanding = () => (
    <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', gap:14, background:'rgba(34,197,94,.05)', border:`1px solid ${C.borderDim}`, borderRadius:14, padding:'13px 14px', animation:'brSlideUp .3s ease both' }}>
      <div style={{ display:'flex', alignItems:'center', gap:11 }}>
        <div style={{ width:42, height:42, borderRadius:12, flexShrink:0, background:'rgba(34,197,94,.1)', border:`1px solid ${C.border}`, display:'flex', alignItems:'center', justifyContent:'center', boxShadow:'0 0 14px rgba(74,222,128,.15)' }}>
      ...
      <button onClick={() => { onActiveChange?.(true); setStep(1); }}
        style={{ flexShrink:0, background:'linear-gradient(135deg,#22C55E,#16A34A)', color:'#fff', border:'none', borderRadius:10, padding:'10px 16px', cursor:'pointer', fontFamily:COND, fontSize:11.5, fontWeight:800, letterSpacing:'.14em', textTransform:'uppercase', boxShadow:'0 4px 18px rgba(34,197,94,.35)', animation:'brGlowPulse 2.8s ease-in-out infinite' }}>
        Request
      </button>
    </div>
  );
  ...
  const renderPickup = () => (
    <div style={{ display:'flex', flexDirection:'column', gap:10, animation:'brSlideUp .3s ease both' }}>
      {(showGeoAlert || geoLoading || geoErr) && (
        <div style={{ background: geoErr ? C.redDim : C.greenDim, border:`1px solid ${geoErr ? 'rgba(248,113,113,.3)' : C.border}`, borderRadius:10, padding:'11px 13px', display:'flex', alignItems:'center', gap:9, animation:'brFadeIn .2s ease both' }}>
          ...
          {!geoLoading && (
            <div style={{ display:'flex', gap:6 }}>
              {!geoErr && <button onClick={handleGeoAllow} style={{ background:C.green, border:'none', borderRadius:7, padding:'5px 10px', cursor:'pointer', fontFamily:COND, fontSize:9.5, fontWeight:800, letterSpacing:'.1em', color:'#fff' }}>Allow</button>}
              <button onClick={() => { setShowGeoAlert(false); clearGeoErr(); }} style={{ background:'rgba(255,255,255,.06)', border:'none', borderRadius:7, padding:'5px 8px', cursor:'pointer', color:C.dim }}>
                <Ico n="x" size={11}/>
              </button>
            </div>
          )}
      ...
      <PlaceInput label="Pickup Address" placeholder="Where are you?" value={pickup} onChange={setPickupVal} autoFocus onLocationRequest={() => { clearGeoErr(); setShowGeoAlert(true); }}/>
  ...
  const renderDropoff = () => (
    <div style={{ display:'flex', flexDirection:'column', gap:10, animation:'brSlideUp .3s ease both' }}>
      <PlaceInput label="Drop-off Address" placeholder="Where to?" value={dropoff} onChange={setDropoffVal} autoFocus/>
      {isLoadingData && (
        <div style={{ display:'flex', alignItems:'center', gap:9, padding:'9px 12px', background:C.faint, border:`1px solid ${C.borderDim}`, borderRadius:10 }}>
          <Spinner size={14}/>
      ...
      {dataError && !isLoadingData && (
        <div style={{ display:'flex', alignItems:'center', gap:8, padding:'9px 12px', background:C.redDim, border:`1px solid rgba(248,113,113,.2)`, borderRadius:10 }}>
          <Ico n="warn" size={13} color={C.red}/>
          <span style={{ fontFamily:MONO, fontSize:9.5, color:C.red }}>{dataError}</span>
        </div>
      )}
      <PrimaryBtn disabled={!dropoff.trim() || isLoadingData || !!dataError} loading={isLoadingData} onClick={() => setStep(3)}>
      ...
      {rideOptions.map(ride => {
        ...
        return (
          <button key={ride.id} className="br-ride-row"
            ...
            style={{ display:'flex', alignItems:'center', gap:11, padding:'10px 13px', borderRadius:11, background: active ? 'rgba(34,197,94,.12)' : C.faint, border:`1px solid ${active ? C.green : C.borderDim}`, cursor:'pointer', textAlign:'left' }}>
            <div style={{ width:36, height:36, borderRadius:10, flexShrink:0, background: active ? 'rgba(34,197,94,.15)' : 'rgba(255,255,255,.05)', border:`1px solid ${active ? C.border : C.borderDim}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
              <Ico n={getRideIcon(ride.id)} size={17} color={active ? C.greenBright : C.dim}/>
            </div>
            <div style={{ flex:1, minWidth:0 }}>
              <div style={{ fontFamily:COND, fontSize:12.5, fontWeight:800, letterSpacing:'.06em', color: active ? '#fff' : 'rgba(255,255,255,.75)', marginBottom:2 }}>{ride.label}</div>
              <div style={{ display:'flex', alignItems:'center', gap:7 }}>
                {etaNone
                  ? <span style={{ fontFamily:MONO, fontSize:8, color:C.amber }}>No ETA</span>
                  : <span style={{ fontFamily:MONO, fontSize:8, color: etaStale ? C.dim : C.greenSoft }}><Ico n="clock" size={9} color={etaStale ? C.dim : C.greenSoft}/> {ride.eta}</span>
                }
                <span style={{ fontFamily:MONO, fontSize:8, color:C.fade }}>· {ride.capacity}</span>
              </div>
            </div>
            <div style={{ fontFamily:MONO, fontSize:16, fontWeight:700, color: active ? C.greenBright : 'rgba(255,255,255,.6)' }}>${Number(ride.total).toFixed(2)}</div>
          </button>
        );
      })}
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:8, animation:'brSlideUp .3s ease both' }}>
      {[
        ...
        <button key={w.id} className="br-pm-row"
          ...
          style={{ display:'flex', alignItems:'center', gap:11, padding:'12px 14px', borderRadius:11, background:C.faint, border:`1px solid ${C.borderDim}`, cursor:'pointer', transition:'all .15s', textAlign:'left' }}>
          <div style={{ width:38, height:38, borderRadius:10, flexShrink:0, background:'rgba(34,197,94,.08)', border:`1px solid ${C.borderDim}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
  ...
  const renderTime = () => (
    <div style={{ display:'flex', flexDirection:'column', gap:10, animation:'brSlideUp .3s ease both' }}>
      ...
      <div>
        <div style={{ fontFamily:MONO, fontSize:8.5, fontWeight:700, color:C.dim, letterSpacing:'.1em', textTransform:'uppercase', marginBottom:7 }}>Select Hour</div>
          ...
          {hours.map(h => {
            ...
            return (
              <div key={h} className={`br-hour${sel ? ' sel-h' : ''}${dis ? ' dis-h' : ''}`}
                ...
                style={{ textAlign:'center', padding:'7px 3px', fontSize:10.5, fontFamily:MONO, fontWeight:500, color: dis ? 'rgba(255,255,255,.18)' : sel ? '#0A1A0C' : '#fff', background: sel ? C.indigo : 'transparent', opacity: dis ? 0.3 : 1, cursor: dis ? 'not-allowed' : 'pointer' }}>
                {lbl}
              </div>
            );
          })}
      ...
      {selHour !== null && (
        <div style={{ animation:'brFadeIn .22s ease both' }}>
          <div style={{ fontFamily:MONO, fontSize:8.5, fontWeight:700, color:C.dim, letterSpacing:'.1em', textTransform:'uppercase', marginBottom:7 }}>Select Minute</div>
            ...
            {[0, 15, 30, 45].map(m => {
              ...
              return (
                <button key={m} type="button" className="br-min-btn"
                  ...
                  disabled={dis}
                  style={{ flex:1, padding:'10px 0', borderRadius:10, border:`1.5px solid ${sel ? C.indigo : C.borderFaint}`, background: sel ? C.indigo : 'transparent', cursor: dis ? 'not-allowed' : 'pointer', opacity: dis ? 0.3 : 1, fontFamily:MONO, fontSize:12, fontWeight:600, color: sel ? '#0A1A0C' : '#fff' }}>
                  :{pad(m)}
                </button>
              );
            })}
      ...
      {scheduledAt && (
        <div style={{ animation:'brFadeIn .2s ease both' }}>
          ...
          <PrimaryBtn onClick={() => setStep(7)}>Confirm Time <Ico n="fwd" size={13}/></PrimaryBtn>
        </div>
      )}
    </div>
  );
  ...
  const renderDiscount = () => (
    <div style={{ display:'flex', flexDirection:'column', gap:10, animation:'brSlideUp .3s ease both' }}>
      ...
      {promoDiscount && (
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'8px 12px', background:C.greenDim, border:`1px solid ${C.border}`, borderRadius:9, fontFamily:MONO, fontSize:9.5, color:C.greenBright }}>
  ...
  const renderPayment = () => (
    <div style={{ display:'flex', flexDirection:'column', gap:8, animation:'brSlideUp .3s ease both' }}>
      <div style={{ display:'flex', alignItems:'baseline', justifyContent:'space-between', padding:'10px 13px', background:C.faint, border:`1px solid ${C.borderDim}`, borderRadius:10 }}>
      ...
      {payError && (
        <div style={{ display:'flex', alignItems:'center', gap:7, padding:'9px 12px', background:C.redDim, border:`1px solid rgba(248,113,113,.2)`, borderRadius:9, fontFamily:MONO, fontSize:9.5, color:C.red }}>
      ...
      {[
        ...
        <button key={pm.id} className="br-pm-row"
          onClick={() => {
            ...
            if (pm.id === 'cashapp') setStep(9);
            if (pm.id === 'cash')    setStep(10);
          }}
          style={{ display:'flex', alignItems:'center', gap:11, padding:'11px 13px', borderRadius:11, background: selectedPayment === pm.id ? 'rgba(34,197,94,.10)' : C.faint, border:`1px solid ${selectedPayment === pm.id ? C.green : C.borderDim}`, cursor:'pointer', textAlign:'left' }}>
          <div style={{ width:36, height:36, borderRadius:10, flexShrink:0, background:'rgba(34,197,94,.07)', border:`1px solid ${C.borderDim}`, display:'flex', alignItems:'center', justifyContent:'center' }}>
            <Ico n={pm.icon} size={17} color={selectedPayment === pm.id ? C.greenBright : C.dim}/>
          </div>
          <div style={{ flex:1 }}>
            <div style={{ fontFamily:COND, fontSize:12, fontWeight:800, letterSpacing:'.06em', color: selectedPayment === pm.id ? '#fff' : 'rgba(255,255,255,.65)' }}>{pm.label}</div>
    ...
    return (
      <div style={{ display:'flex', flexDirection:'column', gap:13, animation:'brSuccessIn .5s cubic-bezier(.34,1.2,.64,1) both' }}>
        <div style={{ display:'flex', justifyContent:'center', padding:'8px 0 4px', position:'relative' }}>
          {[1,2].map(i => (
            <div key={i} style={{ position:'absolute', top:'50%', left:'50%', width:72, height:72, borderRadius:'50%', marginLeft:-36, marginTop:-36, border:`1.5px solid ${C.green}`, animation:`brRingOut ${1 + i * 0.4}s ease-out ${i * 0.25}s both` }}/>
          ))}
          <div style={{ width:64, height:64, borderRadius:'50%', background:'rgba(34,197,94,.14)', border:`2px solid ${C.green}`, display:'flex', alignItems:'center', justifyContent:'center', animation:'brCheckPop .55s cubic-bezier(.34,1.4,.64,1) .1s both', boxShadow:`0 0 30px rgba(74,222,128,.38)`, position:'relative', zIndex:1 }}>
        ...
        {p && (
          <div style={{ background:C.faint, border:`1px solid ${C.borderDim}`, borderRadius:11, padding:'11px 13px', display:'flex', flexDirection:'column', gap:8, animation:'brFadeIn .4s ease .25s both' }}>
            {[
              ...
              <div key={i} style={{ display:'flex', alignItems:'flex-start', gap:9 }}>
                <span style={{ fontFamily:COND, fontSize:8, fontWeight:800, letterSpacing:'.12em', color:C.dim, textTransform:'uppercase', minWidth:42, paddingTop:1 }}>{r.lbl}</span>
                <span style={{ fontFamily:MONO, fontSize:9.5, color:'rgba(255,255,255,.7)', flex:1, wordBreak:'break-all', lineHeight:1.45 }}>{r.val}</span>
              </div>
            ))}
          </div>
        )}
  ...
  return (
    <>
      <style>{KF}</style>
      <div style={{ background:'rgba(5,10,6,.85)', backdropFilter:'blur(16px)', border:`1px solid ${C.border}`, borderRadius:17, padding:'13px 13px 15px', display:'flex', flexDirection:'column', gap:11, boxShadow:'0 8px 40px rgba(0,0,0,.55), 0 0 0 1px rgba(34,197,94,.06)', userSelect:'none' }}>
        <CardHeader step={step} onBack={back} onReset={reset} pickup={pickup} dropoff={dropoff} rideLabel={selectedQuote?.label || null}/>
