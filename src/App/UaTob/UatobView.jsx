...
// Fallback hash-based position when no GPS available
...
function statusColor(status) {
  ...
  if (s === 'online' || s === 'available') return '#16A34A';   // green
  if (s === 'offline')                     return '#111827';   // black
  return '#2563EB';                                            // blue — busy/in_trip/etc.
}
...
export default function MapView({
  ...
  const driverPins = useMemo(() => liveDrivers.map(d => {
    ...
    return { id: d.id, name: d.name || 'Driver', status: d.status || 'offline', pos, color: statusColor(d.status) };
  }), [liveDrivers]);
  ...
  const activeColor = statusColor('online');
  ...
  const isHeadingToDropoff = rideStatus === 'in_progress';
  const isWaitingForPickup = ['driver_assigned', 'arrived'].includes(rideStatus);
  const isOnTrip           = rideStatus === 'in_progress';
  ...
  return (
    <div style={{
      ...
      {/* ── Live driver pins (idle view) ─────────────────────────────── */}
      {!isTracking && driverPins.map((d, i) => (
        <div
          key={d.id}
          title={`${d.name} · ${statusLabel(d.status)}`}
          style={{
            position:       'absolute',
            left:           `${safeCoord(d.pos.x)}%`,
            top:            `${safeCoord(d.pos.y)}%`,
      ...
      {isTracking && driverPos && assignedDriver && (
        <>
          <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
            <defs>
              <linearGradient id="driverRouteGrad" x1="0%" y1="0%" x2="100%" y2="0%">
                <stop offset="0%"   stopColor={activeColor} stopOpacity=".95"/>
                <stop offset="100%" stopColor={activeColor} stopOpacity=".35"/>
              </linearGradient>
            </defs>
            <line
          ...
          <div style={{ position: 'absolute', left: `${safeCoord(driverPos.x)}%`, top: `${safeCoord(driverPos.y)}%`, transform: 'translate(-50%,-50%)', transition: 'all 5s linear', zIndex: 5 }}>
            <div style={{ position: 'absolute', inset: '-10px', borderRadius: '999px', background: `${activeColor}22`, filter: 'blur(10px)' }}/>
            <div style={{ width: '50px', height: '50px', background: `linear-gradient(145deg,${activeColor},${activeColor}dd)`, borderRadius: '18px', display: 'flex', alignItems: 'center', justifyContent: 'center', border: '3px solid #fff', boxShadow: `0 14px 30px ${activeColor}40`, animation: 'pulse 2s ease-in-out infinite', position: 'relative' }}>
          ...
          <div style={{ position: 'absolute', bottom: '16px', left: '50%', transform: 'translateX(-50%)', background: 'rgba(255,255,255,.92)', backdropFilter: 'blur(18px)', border: '1px solid rgba(229,231,235,.9)', borderRadius: '999px', padding: '12px 18px', display: 'flex', alignItems: 'center', gap: '10px', whiteSpace: 'nowrap', boxShadow: '0 12px 30px rgba(0,0,0,.10)', maxWidth: '92%', overflow: 'hidden', zIndex: 6 }}>
            <div style={{ width: '9px', height: '9px', background: activeColor, borderRadius: '999px', animation: 'pulse 1.6s ease-in-out infinite', flexShrink: 0 }}/>
            <span style={{ fontSize: '14px', fontWeight: 800, color: '#111827', overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {getStatusMsg()}
            </span>
            {isWaitingForPickup && (
              <span style={{ background: activeColor, color: '#fff', borderRadius: '999px', padding: '5px 12px', fontSize: '12px', fontWeight: 800, fontFamily: '"JetBrains Mono",monospace', flexShrink: 0, boxShadow: `0 6px 16px ${activeColor}35` }}>
      ...
      {/* Static dropoff pin (no tracking) */}
