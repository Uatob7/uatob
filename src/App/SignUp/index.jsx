...
// ── localStorage helpers (read-only in component) ─────────────────────
const LS_KEYS = {
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ position: 'relative' }}>
        ...
        {suffix && !isPw && (
          <div style={{ position: 'absolute', right: 13, top: '50%', transform: 'translateY(-50%)', fontSize: 12, color: C.textDim, fontWeight: 600, pointerEvents: 'none' }}>{suffix}</div>
        )}
      </div>
      {error && (
...
function StepDocuments({ data, setData, errors, uid }) {
  ...
  const uploadFile = useCallback(async (slot, file) => {
    if (!uid) { console.warn('No UID yet — cannot upload'); return; }
    let localPreview = '';
    ...
    try {
      ...
      await new Promise((resolve, reject) => {
        task.on('state_changed', snap => { setSlot(slot, { progress: Math.round((snap.bytesTransferred / snap.totalBytes) * 100) }); }, reject, resolve);
      });
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 10 }}>
        <div style={{ flex: 1 }}><UploadBox label="Front Side" hint="Tap to upload photo" icon={Camera} uploaded={data.licenseFront} previewUrl={preview('licenseFront')} uploading={uploadState.licenseFront.uploading} progress={uploadState.licenseFront.progress} error={errors?.licenseFront} onFileSelect={f => uploadFile('licenseFront', f)} onRemove={() => removeSlot('licenseFront')}/></div>
        <div style={{ flex: 1 }}><UploadBox label="Back Side"  hint="Tap to upload photo" icon={Camera} uploaded={data.licenseBack}  previewUrl={preview('licenseBack')}  uploading={uploadState.licenseBack.uploading}  progress={uploadState.licenseBack.progress}  error={errors?.licenseBack}  onFileSelect={f => uploadFile('licenseBack', f)}  onRemove={() => removeSlot('licenseBack')}/></div>
      </div>
...
function StepVerify({ accountData, contactData, vehicleData, docData }) {
  ...
  return (
    <div>
      ...
      <Section title="Account"   icon={User}     items={[{ label: 'Name', val: `${accountData.firstName} ${accountData.lastName}` }, { label: 'Email', val: accountData.email }]}/>
      <Section title="Contact"   icon={Phone}    items={[{ label: 'Phone', val: contactData.phone }, { label: 'Address', val: contactData.address }, { label: 'City / State', val: contactData.city && contactData.state ? `${contactData.city}, ${contactData.state} ${contactData.zip}` : '' }]}/>
      <Section title="Vehicle"   icon={Car}      items={[{ label: 'Vehicle', val: vehicleData.make && vehicleData.model ? `${vehicleData.year} ${vehicleData.make} ${vehicleData.model}` : '' }, { label: 'Color', val: vehicleData.color, swatch: colorMeta?.hex }, { label: 'Plate', val: vehicleData.plate?.toUpperCase() }, { label: 'Tiers', val: vehicleData.rideTypes?.map(r => r.charAt(0).toUpperCase() + r.slice(1)).join(', ') }]}/>
      <Section title="Documents" icon={FileText} items={[{ label: "Driver's License", val: (docData.licenseFront && docData.licenseBack) ? '✓ Uploaded' : 'Pending' }, { label: 'Registration', val: docData.registration ? '✓ Uploaded' : 'Pending' }, { label: 'Insurance', val: docData.insurance ? '✓ Uploaded' : 'Pending' }, { label: 'Profile Photo', val: docData.profilePhoto ? '✓ Uploaded' : 'Pending' }]}/>

      {(docData.licenseFrontUrl || docData.profilePhotoUrl) && (
        <div style={{ marginBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, fontWeight: 700, color: C.textMid, marginBottom: 10, letterSpacing: '1.5px', textTransform: 'uppercase', fontFamily: "'Barlow Condensed', sans-serif", paddingLeft: 4 }}>
            <Eye size={11} color={C.textDim} strokeWidth={2.2}/>Document Previews
          </div>
...
export default function UaTobDriverSignup({ uid, driverSignUp }) {
  ...
  // ── Redirect on approved status ──────────────────────────────────
  useEffect(() => {
    const driverSignUpStatus = driverSignUp?.status;
    const driversStatus      = drivers?.[0]?.status;
    if (['approved', 'online'].includes(driverSignUpStatus) || ['approved', 'online'].includes(driversStatus)) {
      router.push('/driver');
    }
  ...
  useEffect(() => {
    ...
    if (driver.contact)   setContactData(d => ({ ...d, ...driver.contact }));
    if (driver.vehicle)   setVehicleData(d => ({ ...d, ...driver.vehicle }));
    if (driver.documents) setDocData(d => ({ ...d, ...driver.documents }));
  }, [drivers]);
  ...
  const goNext = async () => {
    ...
    const formData = { accountData, contactData, vehicleData, docData };
    const result   = await advance(step, formData);
    ...
    // step 5 submit — hook sets submitted=true, no animation needed
    if (step === 5) return;
      ...
      <div style={{ maxWidth: 560, margin: '0 auto', padding: '0 16px 130px', minHeight: '100vh' }}>
        ...
        <div style={{ padding: '26px 0 22px', display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          {(step > 1 || accountData.firstName) && (
            <button
              ...
              style={{ background: 'none', border: `1px solid ${C.border}`, borderRadius: 100, padding: '7px 12px', fontSize: 11, fontWeight: 700, color: C.textDim, cursor: 'pointer', fontFamily: "'Barlow Condensed', sans-serif", letterSpacing: '.5px', textTransform: 'uppercase', transition: 'all .2s' }}
        ...
        {hookError && (
          <div style={{ background: 'rgba(220,38,38,.06)', border: '1px solid rgba(220,38,38,.25)', borderRadius: 14, padding: '13px 16px', marginBottom: 18, display: 'flex', gap: 10, alignItems: 'center', animation: 'errorShake .4s ease' }}>
            ...
            <div style={{ fontSize: 13, color: C.red, fontWeight: 600 }}>{hookError}</div>
          </div>
        )}
