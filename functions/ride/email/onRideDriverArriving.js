...
exports.onRideDriverArriving = onDocumentUpdated(
  ...
  async (event) => {
    try {
      ...
      if (after.status !== "driver_arriving") return null;
      if (after.emailSent_driver_arriving) return null;
      ...
      const eta   = after.driverEtaMin ? `~${after.driverEtaMin} min` : "very soon";
      const year  = new Date().getFullYear();

      const html = `
<!DOCTYPE html><html lang="en"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Driver Arriving</title>
...
<table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:24px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,.08);">
  ...
  <tr><td align="center" style="background:linear-gradient(135deg,#15803D,#16A34A 55%,#22C55E);padding:52px 32px 44px;">
    ...
    <h1 class="hero-title" style="margin:0 0 12px;font-size:32px;font-weight:900;color:#fff;">${esc(driverName)} is almost there</h1>
  ...
  <tr><td class="content-pad" style="padding:36px 32px;">
    <p style="font-size:16px;color:#111827;line-height:1.7;">Hey <strong>${esc(name)}</strong>! Your driver is almost at your pickup location. Head outside now.</p>

    <div style="background:#fffbeb;border:2px solid #fde68a;border-radius:16px;padding:24px;margin-bottom:24px;text-align:center;">
      <div style="font-size:32px;margin-bottom:8px;">⏱️</div>
      <p style="margin:0;font-size:18px;font-weight:800;color:#111827;">Arriving in ${esc(eta)}</p>
      ${vehicle ? `<p style="margin:8px 0 0;font-size:14px;color:#6B7280;">Look for <strong>${esc(vehicle)}${plate ? ` · ${esc(plate)}` : ""}</strong></p>` : ""}
    </div>
  ...
  <tr><td style="padding:24px 32px;text-align:center;background:#f3f4f6;border-top:1px solid #e5e7eb;">
    ...
    <div style="font-size:12px;color:#9CA3AF;">© ${year} UaTob. All rights reserved.</div>
      ...
      await sgMail.send({
        ...
        subject: `📍 ${driverName} is almost there — ${eta} away`,
        text: `Hey ${name}! Your driver is almost there, arriving in ${eta}. ${vehicle ? `Look for ${vehicle}${plate ? ` (${plate})` : ""}.` : ""} Track at https://uatob.com`,
        html,
      });

      await event.data.after.ref.update({ emailSent_driver_arriving: true });
      console.log(`✅ [driver_arriving] email → ${email}`);
    } catch (err) {
      console.error("❌ [driver_arriving]", err.message);
    }
    return null;
  }
);