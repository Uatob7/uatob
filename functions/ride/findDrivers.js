...
exports.findDrivers = onSchedule(
  ...
  async () => {
    ...
    // Get rides searching for drivers
    ...
    console.log(
      `[findDriversScheduled] processing ${ridesSnap.size} ride(s)`
    );

    // Get online drivers once
    ...
    const tasks = ridesSnap.docs.map(async (rideDoc) => {
      ...
      if (pickupLat == null || pickupLng == null) {
        console.warn(
          `[findDriversScheduled] missing coords for ${rideDoc.id}`
        );
        return;
      }

      // Compute sorted drivers
      ...
      console.log(
        `✅ Ride ${rideDoc.id} matched with ${topDrivers.length} drivers`
      );
    });
