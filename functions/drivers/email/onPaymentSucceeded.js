...
// Finds rides in "searching_driver" state and emails all offline drivers who
// haven't been emailed yet for that ride.
          ...
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr>
            <td align="center" style="padding:32px 36px 24px;border-bottom:1px solid #1f1f1f;">
              ...
              ${epm ? `<p style="margin:8px 0 0;font-family:'Courier New',monospace;font-size:12px;
                         ...
                         <span style="color:#4ADE80;">80% split</span></p>` : ""}
            </td>
          </tr></table>
  ...
  async () => {
    try {
      ...
      for (const rideDoc of ridesSnap.docs) {
        ...
        const newDrivers = drivers.filter((d) => !alreadyNotified[d.uid]);

        if (newDrivers.length === 0) {
          console.log(`[notifyOfflineDrivers] Ride ${rideId} — all offline drivers already notified`);
          continue;
        }

        console.log(
          `[notifyOfflineDrivers] Ride ${rideId} — emailing ${newDrivers.length} new offline driver(s)`
        );
