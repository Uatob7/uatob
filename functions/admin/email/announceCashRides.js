...
// One-time HTTP Cloud Function — call it once from the admin dashboard or curl.
// Emails every approved driver announcing that cash rides are now live.
// Tracks how many times the blast has been sent in Firestore: Admin/announcements
...
// ─────────────────────────────────────────────────────────────
// Firestore ref for the send-count flag
// Doc: Admin/announcements  field: cashRidesSentCount
// ─────────────────────────────────────────────────────────────
const ANNOUNCE_REF = db.collection("Admin").doc("announcements");

// ─────────────────────────────────────────────────────────────
// Brand SVGs (same as notifyOfflineDrivers)
// ─────────────────────────────────────────────────────────────
...
<table width="100%" cellpadding="0" cellspacing="0" role="presentation"
        ...
        <td style="background-color:#111111;border-radius:20px;
          ...
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr>
            <td style="background:linear-gradient(135deg,#052e16 0%,#14532d 50%,#166534 100%);
              ...
              <p style="margin:0;font-family:'Courier New',monospace;font-size:13px;
                ...
                You collect the money directly at pickup. Simple as that.
              </p>
            </td>
          </tr></table>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr>
            ...
            <td width="33%" align="center" class="step-td"
              ...
              <p style="margin:0;font-family:Georgia,serif;font-size:14px;
                        color:#d1d5db;line-height:1.5;">
                The app shows the exact fare. Collect it from the rider at pickup.
              </p>
            </td>
            <td width="33%" align="center" class="step-td"
              ...
              <p style="margin:0 0 6px;font-family:'Courier New',monospace;font-size:10px;
                        font-weight:700;color:#4ADE80;letter-spacing:1.5px;">KEEP YOUR CUT</p>
              <p style="margin:0;font-family:Georgia,serif;font-size:14px;
                        color:#d1d5db;line-height:1.5;">
                75% is yours to keep immediately. No waiting for a payout.
              </p>
            </td>
          </tr></table>
                  ...
                  <table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr>
                    ...
                    <td valign="middle">
                      ...
                      <p style="margin:0;font-family:Georgia,serif;font-size:14px;
                        ...
                        so you always know what you're accepting.
                      </p>
                    </td>
                  </tr></table>
                </td>
          ...
          <table width="100%" cellpadding="0" cellspacing="0" role="presentation"><tr>
            <td style="padding:16px 36px;background-color:#0d0d0d;border-top:1px solid #1f1f1f;">
              <p style="margin:0;font-family:'Courier New',monospace;font-size:11px;
                ...
                ANNOUNCEMENT &nbsp;<span style="color:#4ADE80;">AD #${sendCount}</span>
              </p>
            </td>
          </tr></table>
  ...
  const text =
    ...
    `- Collect the exact fare shown in the app from the rider at pickup\n` +
    `- Keep 75% immediately — same split as card rides, no payout wait\n` +
  ...
  return {
    ...
    subject: "💵 New: Riders can now pay cash — here's how it works",
    text,
    html,
  };
}
...
// ─────────────────────────────────────────────────────────────
exports.announceCashRides = onCall(
  {
    region:  "us-east1",
  ...
  async (request) => {
    ...
    // ── 1. Atomically read + increment send count ──────────────────────
    const newCount = await db.runTransaction(async (txn) => {
      const snap = await txn.get(ANNOUNCE_REF);
      const prev = snap.exists ? (snap.data().cashRidesSentCount ?? 0) : 0;
      const next = prev + 1;
      txn.set(
        ANNOUNCE_REF,
        {
          cashRidesSentCount:  next,
    ...
    console.log(`[announceCashRides] Send #${newCount}`);

    // ── 2. Pull all approved drivers with an email ─────────────────────
    ...
    if (drivers.length === 0) {
      console.log("[announceCashRides] No drivers with email found");
      return { success: true, sent: 0, failed: 0, sendCount: newCount };
    }

    console.log(`[announceCashRides] Emailing ${drivers.length} driver(s)`);
    ...
    results.forEach((r, i) => {
      if (r.status === "rejected") {
        console.error(
          `[announceCashRides] Failed for ${drivers[i]?.email}:`,
          r.reason?.message
        );
      }
    });

    // ── 4. Write final send stats back ─────────────────────────────────
    await ANNOUNCE_REF.set(
      {
        cashRidesLastSentDriverCount: drivers.length,
    ...
    console.log(`[announceCashRides] Done — AD #${newCount} | ${sent} sent, ${failed} failed`);
    return { success: true, sent, failed, sendCount: newCount };
  }
);