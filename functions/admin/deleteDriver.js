const { onCall, HttpsError } = require("firebase-functions/v2/https");
const admin = require("firebase-admin");

if (!admin.apps.length) admin.initializeApp();
const db = admin.firestore();

/**
 * deleteDriver
 *
 * Admin-callable function that completely removes a driver from the app.
 * This includes:
 *   1. Delete the driver document from the Drivers collection
 *   2. Remove driver references from any active/completed rides
 *   3. Delete any reviews associated with the driver
 *   4. Log the deletion for audit purposes
 *
 * Required caller data:
 *   driverUid {string} — The UID of the driver to delete
 *
 * Returns:
 *   { success: true, driverUid, deletedCounts: { rides, reviews } }
 */
exports.deleteDriver = onCall({ region: "us-central1" }, async (request) => {
  try {
    const { driverUid } = request.data ?? {};

    // ── Validation ─────────────────────────────────────────────────────────
    if (!driverUid) {
      throw new HttpsError("invalid-argument", "Missing driverUid");
    }

    // ── Check if driver exists ─────────────────────────────────────────────
    const driverRef = db.collection("Drivers").doc(driverUid);
    const driverSnap = await driverRef.get();

    if (!driverSnap.exists) {
      throw new HttpsError("not-found", "Driver not found");
    }

    const driverData = driverSnap.data();
    const driverName = `${driverData.firstName || ""} ${driverData.lastName || ""}`.trim() || driverUid;

    // ── Find and clean up rides with this driver ───────────────────────────
    let ridesDeleted = 0;
    const ridesSnapshot = await db
      .collection("Rides")
      .where("driverUid", "==", driverUid)
      .get();

    if (!ridesSnapshot.empty) {
      const batch = db.batch();
      ridesSnapshot.docs.forEach((rideDoc) => {
        batch.update(rideDoc.ref, {
          driverUid: null,
          driverName: null,
          status: rideDoc.data().status === "in_progress" 
            ? "cancelled"  // Cancel in-progress rides
            : rideDoc.data().status,  // Keep other statuses
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
          reason_cancelled: `Driver deleted by admin: ${driverName}`,
        });
        ridesDeleted++;
      });
      await batch.commit();
      console.log(`✅ Updated ${ridesDeleted} rides for deleted driver: ${driverUid}`);
    }

    // ── Find and delete reviews for this driver ────────────────────────────
    let reviewsDeleted = 0;
    const reviewsSnapshot = await db
      .collection("Reviews")
      .where("driverUid", "==", driverUid)
      .get();

    if (!reviewsSnapshot.empty) {
      const batch = db.batch();
      reviewsSnapshot.docs.forEach((reviewDoc) => {
        batch.delete(reviewDoc.ref);
        reviewsDeleted++;
      });
      await batch.commit();
      console.log(`✅ Deleted ${reviewsDeleted} reviews for driver: ${driverUid}`);
    }

    // ── Delete the driver document ─────────────────────────────────────────
    await driverRef.delete();
    console.log(`✅ Driver deleted: ${driverUid} (${driverName})`);

    return {
      success: true,
      driverUid,
      driverName,
      deletedCounts: {
        rides: ridesDeleted,
        reviews: reviewsDeleted,
      },
    };
  } catch (err) {
    console.error("❌ deleteDriver error:", err);
    if (err instanceof HttpsError) throw err;
    throw new HttpsError("internal", err.message || "Failed to delete driver");
  }
});
